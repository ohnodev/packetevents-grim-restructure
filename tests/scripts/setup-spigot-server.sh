...
MC_VERSION="1.21.1"
SERVER_DIR="tests/runs/spigot-1.21.1"
JAVA_BIN="java"
REBUILD="0"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "Unknown arg: $1"
      echo "Usage: $0 [--mc-version 1.21.1] [--server-dir tests/runs/spigot-1.21.1] [--java-bin /path/to/java] [--rebuild]"
      exit 1
      ;;
  esac
done
...
echo "[setup] Next: ./tests/scripts/install-packetevents-bukkit-dev-jar.sh --server-dir ${SERVER_DIR}"
